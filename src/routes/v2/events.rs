...
pub async fn chart_data(
    ...
    match get_feedback_list("dynamic_threat", "associative", limit, &database.pool).await {
        Ok(feedbacks) => {
            let death_counts = get_death_counts(limit, &database.pool)
                .await
                .unwrap_or_default();
            let citation_counts = get_citation_counts(limit, &database.pool)
                .await
                .unwrap_or_default();
            let round_durations = get_round_durations(limit, &database.pool)
                .await
                .unwrap_or_default();
            let total_player_counts = get_player_counts(limit, &database.pool)
                .await
                .unwrap_or_default();
